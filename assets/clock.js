...
clock.flipClock = function(identifierUp, identifierDown, val, type) {
...
clock.run = function() {
	...
	if (hour !== hour_cur) {
		clock.flipClock('#clock-front-hours-up', '#clock-front-hours-down', hour, 'double');
		hour_cur = hour;
	}

	if (minR !== minR_cur) {
		clock.flipClock('#clock-front-minutes-up-right', '#clock-front-minutes-down-right', minR, 'single');
		minR_cur = minR;
	}

	if (minL !== minL_cur) {
		clock.flipClock('#clock-front-minutes-up-left', '#clock-front-minutes-down-left', minL, 'single');
		minL_cur = minL;
	}

	if (secR !== secR_cur) {
		clock.flipClock('#clock-front-seconds-up-right', '#clock-front-seconds-down-right', secR, 'single');
		secR_cur = secR;
	}

	if (secL !== secL_cur) {
		clock.flipClock('#clock-front-seconds-up-left', '#clock-front-seconds-down-left', secL, 'single');
		secL_cur = secL;
	}
};
